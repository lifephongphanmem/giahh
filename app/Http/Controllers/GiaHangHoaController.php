...
class GiaHangHoaController extends Controller
{
    public function store(Request $request)
    {
        ...
        $mahh =$inputs['mahh'];
        $mahs = $inputs['mahs'];
        $m_kt=GiaHangHoa::where('mahh',$mahh)->where('mahs',$mahs)->get();
        if(count($m_kt)>0){
            $result = array(
                'status' => 'fail',
    ...
    public function edit(Request $request)
    {
        ...
        if(isset($inputs['id'])){
            ...
            $model = GiaHangHoa::where('id',$inputs['id'])
                ->first();
            //dd($model);
    ...
    public function get_attackfile(Request $request)
    {
        ...
        $model = HsGiaHangHoa::find($inputs['id']);
