...
class GiaHhXnkDefaultController extends Controller
{
    public function getpnhomhh(Request $request)
    {
        $result = array(
            'status' => 'fail',
            'message' => 'error',
        ...
        $inputs = $request->all();
        //dd($inputs);
        if(isset($inputs['manhom'])){

            $model = PNhomxnk::where('manhom',$inputs['manhom'])
                ->get();

            //dd($model);
            $result['message'] = '<div id="ttpnhom">';
            if(count($model) > 0){
                $result['message'] .= '<select class="form-control select2me" name="mapnhom" id="mapnhom">';
                $result['message'] .= '<option value="">--Chọn phân nhóm hàng hóa--</option>';
                foreach($model as $tt){
                    $result['message'] .= '<option value="'.$tt->masopnhom.'">'.$tt->tenpnhom.'</option>';
                }
    ...
    public function getloai(Request $request)
    {
        ...
        $inputs = $request->all();
        //dd($inputs);
        if(isset($inputs['masopnhom'])){

            $model = Loaixnk::where('masopnhom',$inputs['masopnhom'])
                ->get();
            //dd($model);
            $result['message'] = '<div id="ttloai">';
            if(count($model) > 0){
                $result['message'] .= '<select class="form-control select2me" name="maloai" id="maloai">';
                $result['message'] .= '<option value="">--Chọn loại hàng hóa--</option>';
                foreach($model as $tt){
                    $result['message'] .= '<option value="'.$tt->masoloai.'">'.$tt->tenloai.'</option>';
                }
                $result['message'] .= '</select>';
                $result['message'] .= '</div>';
                $result['status'] = 'success';
    ...
    public function gethh(Request $request)
    {
        ...
        $inputs = $request->all();
        //dd($inputs);
        if(isset($inputs['masoloai'])){

            $model = DmHhXnk::where('masoloai',$inputs['masoloai'])
                ->get();
            //dd($model);
            $result['message'] = '<div id="tthh">';
            if(count($model) > 0){
                $result['message'] .= '<select class="form-control select2me" name="mahh" id="mahh">';
                $result['message'] .= '<option value="">--Chọn hàng hóa--</option>';
                foreach($model as $tt){
    ...
    public function store(Request $request)
    {
        ...
        if($inputs['mahh']!=''){
            $modelts = new GiaHhXnkDefault();
            $modelts->masoloai = $inputs['masoloai'];
            ...
            $model = GiaHhXnkDefault::where('mahuyen',session('admin')->mahuyen)
                ->get();
            $modeldm = DmHhXnk::all();
    ...
    public function edit(Request $request)
    {
        ...
        if(isset($inputs['id'])){
            ...
            $model = GiaHhXnkDefault::where('id',$inputs['id'])
                ->first();
            //dd($model);
            ...
            $result['message'] .= '<div class="form-group"><label for="selGender" class="control-label">Mã số loại<span class="require">*</span></label>';
            $result['message'] .= '<div><input type="text" name="masoloaiedit" id="masoloaiedit" class="form-control" value="'.$model->masoloai.'"></div>';
            $result['message'] .= '</div>';
            $result['message'] .= '</div>';
            $result['message'] .= '<div class="col-md-6">';
    ...
    public function update(Request $request)
    {
        ...
        if(isset($inputs['id'])){
            ...
            $modelupdate = GiaHhXnkDefault::where('id',$inputs['id'])
                ->first();
            $modelupdate->masoloai = $inputs['masoloai'];
    ...
    public function destroy(Request $request)
    {
        ...
        if(isset($inputs['id'])){
            $modeldel = GiaHhXnkDefault::where('id',$inputs['id'])
                ->delete();
