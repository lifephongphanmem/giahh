...
class GiaHhTnDefaultController extends Controller
{
    public function getpnhomhh(Request $request)
    {
        ...
        if(isset($inputs['manhom'])){
            ...
            $model = PNhomTn::where('manhom',$inputs['manhom'])
                ->get();
            //dd($model);
            $result['message'] = '<div id="ttpnhom">';
            if(count($model) > 0){
                $result['message'] .= '<select class="form-control select2me" name="mapnhom" id="mapnhom">';
                $result['message'] .= '<option value="">--Chọn phân nhóm hàng hóa- dịch vụ--</option>';
                foreach($model as $tt){
    ...
    public function gettthh(Request $request)
    {
        ...
        if(isset($inputs['mapnhom'])){
            ...
            $model = DmHhTn::where('masopnhom',$inputs['mapnhom'])
                ->get();
            //dd($model);
            $result['message'] = '<div id="tthh">';
            if(count($model) > 0){
                $result['message'] .= '<select class="form-control select2me" name="mahh" id="mahh">';
                $result['message'] .= '<option value="">--Chọn hàng hóa- dịch vụ--</option>';
                foreach($model as $tt){
    ...
    public function store(Request $request)
    {
        ...
        if($inputs['mahh']!=''){
            ...
            $model = GiaHhTnDefault::where('mahuyen',session('admin')->mahuyen)
                ->get();
    ...
    public function edit(Request $request)
    {
        ...
        if(isset($inputs['id'])){
            ...
            $model = GiaHhTnDefault::where('id',$inputs['id'])
                ->first();
            //dd($model);
    ...
    public function update(Request $request)
    {
        ...
        if(isset($inputs['id'])){
            ...
            $modelupdate = GiaHhTnDefault::where('id',$inputs['id'])
                ->first();
    ...
    public function destroy(Request $request)
    {
        ...
        if(isset($inputs['id'])){
            $modeldel = GiaHhTnDefault::where('id',$inputs['id'])
                ->delete();
